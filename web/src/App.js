...
export default function App() {
  ...
  setEndStationName(loc.name || null);
    ...
    setSelected(loc);
    // attach available prediction based on current route
    const pred = route === 'seasonal' ? seasonalPredictions[loc.id] : predictions[loc.id];
  ...
  useEffect(() => {
    async function fetchSeasonalPredictions() {
      ...
      try {
        ...
        const months = seasonMonths[selectedSeason] || [3, 4, 5];
        const year = selectedYear;
        
        // Fetch predictions for all months in the season
        ...
        // Average the predictions across months
        ...
        puneLocations.forEach(loc => {
          ...
          if (locPredictions.length > 0) {
            ...
            // Determine overall water quality based on averaged parameters
            const avgDO = avgPred['DO (mg/L)'];
            const avgBOD = avgPred['BOD (mg/L)'];
            const avgpH = avgPred['pH'];
            
            let quality = 'Moderate';
            if (avgDO >= 5 && avgBOD <= 3 && avgpH >= 6.5 && avgpH <= 8.5) {
              quality = 'Good';
            } else if (avgDO < 3 || avgBOD > 6) {
              quality = 'Poor';
            }
            avgPred['Water Quality'] = quality;
            
            locationAverages[loc.id] = avgPred;
          }
        });
        
        setSeasonalPredictions(locationAverages);
      } catch (err) {
  ...
  useEffect(() => {
    if (selected && selected.id) {
      const pred = route === 'seasonal' ? seasonalPredictions[selected.id] : predictions[selected.id];
      setSelected(prev => ({...prev, prediction: pred}));
    }
  }, [predictions, seasonalPredictions, route, selected?.id]);
      ...
      <main className="main" style={{display:'flex', gap:20}}>
            ...
            <div className="home-page">
              <div className="home-card">
                ...
                <div className="home-cta">
                  ...
                  <button className="secondary" onClick={() => setRoute('seasonal')}>Seasonal Variation</button>
                </div>
              </div>
            </div>
          ) : (
        ...
        {route !== 'home' && (
          ...
          <div className="tab-bar">
            ...
            <button onClick={()=>setRoute('seasonal')} className="small primary">Seasonal</button>
          </div>
          {route === 'seasonal' ? (
            <div className="detail-calendar" style={{padding: '16px'}}>
              <h4 style={{marginBottom: '12px', color: '#1e293b'}}>Select Year & Season</h4>
              <div style={{marginBottom: '16px'}}>
                <label style={{display: 'block', marginBottom: '8px', fontSize: '13px', color: '#475569', fontWeight: '500'}}>Year</label>
                <select 
                  value={selectedYear} 
              ...
              <div style={{display: 'flex', flexDirection: 'column', gap: '8px'}}>
                <button 
                  ...
                  className={selectedSeason === 'summer' ? 'season-btn active' : 'season-btn'}
                  style={{
                    padding: '12px 16px',
                    borderRadius: '8px',
                    border: selectedSeason === 'summer' ? '2px solid #f59e0b' : '1px solid #e2e8f0',
                    background: selectedSeason === 'summer' ? '#fef3c7' : '#fff',
                    color: selectedSeason === 'summer' ? '#92400e' : '#475569',
                    cursor: 'pointer',
                    fontWeight: selectedSeason === 'summer' ? '600' : '400',
                    transition: 'all 0.2s'
                  }}
                >
                  🌞 Summer (Mar-May)
                ...
                <button 
                  ...
                  className={selectedSeason === 'monsoon' ? 'season-btn active' : 'season-btn'}
                  style={{
                    padding: '12px 16px',
                    borderRadius: '8px',
                    border: selectedSeason === 'monsoon' ? '2px solid #3b82f6' : '1px solid #e2e8f0',
                    background: selectedSeason === 'monsoon' ? '#dbeafe' : '#fff',
                    color: selectedSeason === 'monsoon' ? '#1e40af' : '#475569',
                    cursor: 'pointer',
                    fontWeight: selectedSeason === 'monsoon' ? '600' : '400',
                    transition: 'all 0.2s'
                  }}
                >
                  🌧️ Monsoon (Jun-Sep)
                ...
                <button 
                  ...
                  className={selectedSeason === 'winter' ? 'season-btn active' : 'season-btn'}
                  style={{
                    padding: '12px 16px',
                    borderRadius: '8px',
                    border: selectedSeason === 'winter' ? '2px solid #06b6d4' : '1px solid #e2e8f0',
                    background: selectedSeason === 'winter' ? '#cffafe' : '#fff',
                    color: selectedSeason === 'winter' ? '#0e7490' : '#475569',
                    cursor: 'pointer',
                    fontWeight: selectedSeason === 'winter' ? '600' : '400',
                    transition: 'all 0.2s'
                  }}
                >
                  ❄️ Winter (Oct-Feb)
          ...
          <div style={{marginTop:16}}>
            ...
            {route === 'seasonal' && (
              <div className="selected-date">Season: <strong style={{textTransform: 'capitalize'}}>{selectedSeason} {selectedYear}</strong> (Averaged)</div>
            )}
                {selected ? (
              <div style={{marginTop:12}}>
                ...
                <h4 style={{marginTop:8}}>{route === 'seasonal' ? 'Seasonal Average' : 'Prediction'}</h4>
                {selected.prediction ? (
                  <div>
